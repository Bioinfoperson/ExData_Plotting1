...
missing_data <- Power_data$Sub_metering_1!='?'|Power_data$Sub_metering_2!='?'|Power_data$Sub_metering_3!='?'
Power_data_plot3 <- Power_data[missing_data,]

# Create calender date and time object.
datetime <- as.POSIXlt(paste(Power_data_plot3$Date, Power_data_plot3$Time, sep=" "), "%Y-%m-%d %H:%M:%S")

# Plot in PNG
png("plot3.png",height=480,width=480)
plot(datetime, Power_data_plot3$Sub_metering_1, xlab="", ylab="Energy sub metering", col="black",type="l")
points(datetime, Power_data_plot3$Sub_metering_2,col="red",type="l")
points(datetime, Power_data_plot3$Sub_metering_3,col="blue",type="l")
